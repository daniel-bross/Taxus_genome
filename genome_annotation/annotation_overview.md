# Genome annotations
The annotation of the four genome assemblies (B236-h1, B236-h2, B346-h1, B346-h2) were generated by the following steps:
...
The conda location and environment name should be set in config.cfg. The following packages were used:
...
- a file `entap_config.ini` that stores EnTAP configurations

To generate the protein evidence file:
...
- generate statistics with `awk -f get_gtf_stats.awk <gtf_file>`

## 4. EnTAP
1. install and set up EnTAP according to its [documentation](https://entap.readthedocs.io/en/latest/Getting_Started/introduction.html)
2. depending on where the databases are stored, some lines in `data/entap_config.ini` may have to be changed
3. run `scripts/entap.sh`
