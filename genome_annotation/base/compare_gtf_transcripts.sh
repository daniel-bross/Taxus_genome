...
bedtools intersect -u -sorted \
	-a  <(awk '/^[^#]/ {if ($3 == "transcript") printf("%s\t%d\t%s\t%s\n",$1,int($4)-1,$5,$0);}' $A  | sort -t $'\t' -k1,1 -k2,2n ) \
	-b  <(awk '/^[^#]/ {if ($3 == "transcript") printf("%s\t%d\t%s\t%s\n",$1,int($4)-1,$5,$0);}' $B  | sort -t $'\t' -k1,1 -k2,2n ) > ${BASEDIR}results/gtf_intersect_braker_sr_lr.gtf
